...
class Game:
    ...
    def move(self, dx, dy):
        ...
        if not self.check_collision(next_move_x, next_move_y, self.grid):
    ...
    def check_collision(self, next_move_x, next_move_y, grid):
        for row_index, row in enumerate(self.current_block.shape):
            for col_index, cell in enumerate(row):
                if cell != 0:
                    ...
                    # Verificar si la nueva posición está fuera de los límites de la cuadrícula
                    if (
                        board_col < 0  # Utiliza la propiedad left_boundary
                        or board_col >= grid.col
                        or board_row >= grid.row
                    ):
